...
/**
 * Inspira UI Fluid Cursor adapted for React
 * Based on WebGL 2D Navier-Stokes fluid simulation algorithm:
 * - Dye & Velocity Advection
 * - Vorticity Confinement & Curl
 * - Divergence & Jacobi Pressure Solver
 * - Gradient Subtraction
 * - Custom Amber Colorization matching Portfolio Theme
 */
export default function FluidCursor({
  ...
  useEffect(() => {
    // ── Capability checks (desktop fine pointer & non-reduced motion) ────────
    ...
    class Material {
      ...
      setKeywords(keywords) {
        let hash = 0;
        for (const kw of keywords) hash += kw.length;
    ...
    let dye, velocity, divergence, curlBuf, pressureBuf;
    ...
    function initFramebuffers() {
      ...
      curlBuf    = createFBO(simRes.width, simRes.height, r.internalFormat, r.format, texType, gl.NEAREST);
      pressureBuf = createDoubleFBO(simRes.width, simRes.height, r.internalFormat, r.format, texType, gl.NEAREST);
    }
    ...
    // ── Kinetic Amber Color Generator matching Portfolio Palette ───────────
    function generateColor() {
      // Kinetic Amber base (#f59e0b) with warm highlights
      return {
        r: 0.96 * 0.18,
        g: 0.62 * 0.18,
        b: 0.05 * 0.18,
    ...
    function step(dt) {
      ...
      if (vorticityProgram.uniforms.uCurl) gl.uniform1i(vorticityProgram.uniforms.uCurl, curlBuf.attach(1));
      ...
      // Clear pressure
      clearProgram.bind();
      if (clearProgram.uniforms.uTexture) gl.uniform1i(clearProgram.uniforms.uTexture, pressureBuf.read.attach(0));
      if (clearProgram.uniforms.value) gl.uniform1f(clearProgram.uniforms.value, config.PRESSURE);
      blit(pressureBuf.write);
      pressureBuf.swap();
      ...
      for (let i = 0; i < config.PRESSURE_ITERATIONS; i++) {
        if (pressureProgram.uniforms.uPressure) gl.uniform1i(pressureProgram.uniforms.uPressure, pressureBuf.read.attach(1));
        blit(pressureBuf.write);
        pressureBuf.swap();
      }
      ...
      if (gradienSubtractProgram.uniforms.uPressure) gl.uniform1i(gradienSubtractProgram.uniforms.uPressure, pressureBuf.read.attach(0));
  ...
  return (
    <div className="pointer-events-none fixed top-0 left-0 z-[99999] size-full">
      <canvas
        id="fluid"
        ref={canvasRef}
