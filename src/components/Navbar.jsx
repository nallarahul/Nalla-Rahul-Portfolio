  ...
  return (
    <motion.button
      ...
      className={`relative px-3.5 py-1.5 text-xs font-mono transition-colors duration-200 rounded-full ${
        isActive ? 'text-amber-500 font-semibold' : 'text-steel-400 hover:text-steel-100'
      }`}
    >
      {link.label}
...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      for (let i = sections.length - 1; i >= 0; i--) {
        ...
        if (sec && sec.offsetTop <= scrollPos) { setActiveSection(sections[i]); break; }
      }
    };
  ...
  return (
    <motion.header
      ...
      variants={{
        top:     { paddingTop: '1.5rem', paddingBottom: '1.5rem', backgroundColor: 'rgba(8,8,12,0)' },
        scrolled:{ paddingTop: '0.65rem', paddingBottom: '0.65rem', backgroundColor: 'rgba(8,8,12,0.92)' },
      }}
    ...
    >
      <motion.div
        animate={scrolled
          ? { borderColor: 'rgba(30,32,46,0.7)', borderBottomWidth: '1px' }
          : { borderColor: 'transparent', borderBottomWidth: '1px' }
        }
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          >
            <motion.div
              whileHover={prefersReduced ? {} : { borderColor: 'rgba(245,158,11,0.5)', scale: 1.04 }}
            ...
            <div className="hidden sm:block">
              <span className="block font-heading text-sm font-semibold tracking-tight text-steel-100 group-hover:text-amber-500 transition-colors">{portfolioData.personal.name}</span>
              <span className="block font-mono text-[10px] text-steel-400">{portfolioData.personal.role}</span>
            </div>
          </button>
          ...
          <div className="hidden md:flex items-center gap-3">
            <MagneticCTA onClick={() => scrollToSection('contact')} prefersReduced={prefersReduced} />
          </div>
          ...
          >
            <AnimatePresence mode="wait" initial={false}>
              {mobileMenuOpen
                ? <motion.span key="close" initial={{ rotate: -45, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 45, opacity: 0 }} transition={{ duration: 0.18 }}><X className="w-5 h-5" /></motion.span>
                : <motion.span key="open" initial={{ rotate: 45, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -45, opacity: 0 }} transition={{ duration: 0.18 }}><Menu className="w-5 h-5" /></motion.span>
              }
            </AnimatePresence>
          </motion.button>
        </div>
        ...
        {mobileMenuOpen && (
          <>
            <motion.div
              ...
              className="md:hidden fixed inset-0 top-[56px] bg-bg-dark/60 backdrop-blur-sm z-[-1]"
            ...
            <motion.div
              ...
              transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
              className="md:hidden bg-bg-dark/98 border-b border-bg-border/80 backdrop-blur-xl overflow-hidden"
            >
              <motion.div
                initial="hidden"
                animate="visible"
              ...
              >
                {navLinks.map((link) => (
                  <motion.button
                    ...
                    onClick={() => scrollToSection(link.id)}
                    className={`text-left py-2 border-b border-bg-border/40 flex items-center justify-between ${activeSection === link.id ? 'text-amber-500 font-bold' : 'text-steel-300'}`}
                  >
                ...
                <motion.button
                  variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0, transition: { duration: 0.3, delay: 0.35 } } }}
